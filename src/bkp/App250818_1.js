...
import {
  ...
  Clock, Star, Edit, Trash2, Eye, Download, Filter, X, Save, Phone, Cake, Coffee, Cookie, Sparkles, Gift, ChevronLeft, ChevronRight, Printer, Home, BookOpen
} from 'lucide-react';
  ...
  const [data, setData] = React.useState(() => {
    try {
      ...
      if (savedData) {
        ...
        if (parsedData.pedidos && parsedData.pedidos.some(p => typeof p.produtos === 'string')) {
...
function App() {
  ...
  const Dashboard = () => {
    ...
    const anniversaryCakeAlerts = data.pedidos.filter(pedido => {
        if (Array.isArray(pedido.produtos) && pedido.produtos.some(p => p.nome.toLowerCase().includes('aniversário'))) {
            ...
            return today >= alertDate && today <= new Date(new Date(orderDate).setFullYear(orderDate.getFullYear() + 1));
        }
        return false;
    });
    ...
    return (
      <div className="p-6 space-y-6 bg-gradient-to-br from-pink-50/30 to-rose-50/30 min-h-screen">
        <div className="flex justify-between items-center">
          ...
          <Button onClick={() => window.open('/cardapio.html', '_blank')}><BookOpen className="w-4 h-4" /> Cardápio</Button>
        </div>
  ...
  const renderCurrentPage = () => {
    switch (currentPage) {
      case 'pagina-inicial': return <Dashboard />;
