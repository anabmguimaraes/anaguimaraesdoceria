...
import {
  ...
  Clock, Star, Edit, Trash2, Eye, Download, Filter, X, Save, Phone, Cake, Coffee, Cookie, Sparkles, Gift, ChevronLeft, ChevronRight
} from 'lucide-react';
...
const mockData = {
  ...
  pedidos: [
    { id: 1001, cliente: "Maria Silva", produtos: "Bolo Chocolate, Torta Morango", total: 85.50, data: new Date().toISOString().split('T')[0], status: "Concluído", pagamento: "Cartão", tipo: "Pedido Normal" },
    { id: 1002, cliente: "João Santos", produtos: "Cupcakes (6un)", total: 42.00, data: new Date().toISOString().split('T')[0], status: "Pendente", pagamento: "PIX", tipo: "Pedido Normal" },
    { id: 1003, cliente: "Ana Costa", produtos: "Bolo Aniversário", total: 120.00, data: "2025-09-25", status: "Em Preparo", pagamento: "Dinheiro", tipo: "Aniversário" }
  ],
  produtos: [
...
function App() {
  ...
  const menuItems = [
    { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard }, { id: 'clientes', label: 'Clientes', icon: Users },
    { id: 'pedidos', label: 'Pedidos', icon: ShoppingCart }, { id: 'produtos', label: 'Produtos', icon: Package },
    { id: 'agenda', label: 'Agenda', icon: Calendar }, { id: 'fornecedores', label: 'Fornecedores', icon: Truck },
    { id: 'financeiro', label: 'Financeiro', icon: DollarSign }, { id: 'relatorios', label: 'Relatórios', icon: BarChart3 },
  ];
  ...
  const Dashboard = () => {
    ...
    const upcomingBirthdays = data.clientes.filter(cliente => {
        const birthday = new Date(cliente.aniversario);
        birthday.setFullYear(today.getFullYear());
        return birthday >= today && birthday <= nextMonth;
    });

    const anniversaryCakeAlerts = data.pedidos.filter(pedido => {
        if (pedido.produtos.toLowerCase().includes('aniversário')) {
            const orderDate = new Date(pedido.data);
            const alertDate = new Date(orderDate);
            alertDate.setFullYear(orderDate.getFullYear() + 1);
            alertDate.setMonth(alertDate.getMonth() - 1); // 1 Mês antes
            return today >= alertDate && today <= new Date(orderDate.setFullYear(orderDate.getFullYear() + 1));
        }
        return false;
    });
    ...
    return (
      <div className="p-6 space-y-6 bg-gradient-to-br from-pink-50/30 to-rose-50/30 min-h-screen">
        <div className="flex justify-between items-center">
          ...
          <Button><Plus className="w-4 h-4" /> Novo Pedido</Button>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
            <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2"><AlertTriangle className="text-yellow-500"/>Alertas Importantes</h3>
            <div className="space-y-4">
                {upcomingBirthdays.length > 0 && upcomingBirthdays.map(cliente => (
                    <div key={cliente.id} className="flex items-center gap-3 p-3 rounded-xl bg-yellow-50 border border-yellow-200">
                        <Gift className="w-5 h-5 text-yellow-600"/>
                        <div>
                            <p className="font-medium text-yellow-800">Aniversário de {cliente.nome} se aproximando!</p>
                            <p className="text-sm text-yellow-700">Data: {new Date(cliente.aniversario).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long' })}</p>
                        </div>
                    </div>
                ))}
                {anniversaryCakeAlerts.length > 0 && anniversaryCakeAlerts.map(pedido => (
                     <div key={pedido.id} className="flex items-center gap-3 p-3 rounded-xl bg-blue-50 border border-blue-200">
                        <Cake className="w-5 h-5 text-blue-600"/>
                        <div>
                            <p className="font-medium text-blue-800">Lembrete de Aniversário para {pedido.cliente}</p>
                            <p className="text-sm text-blue-700">Há quase um ano, foi comprado um bolo de aniversário. Que tal entrar em contato?</p>
                        </div>
                    </div>
                ))}
                {upcomingBirthdays.length === 0 && anniversaryCakeAlerts.length === 0 && (
  ...
  const Pedidos = () => {
    ...
    const resetForm = () => {
        ...
        setFormData({ cliente: '', produtos: '', total: '', pagamento: 'Cartão', status: 'Pendente', tipo: 'Pedido Normal', data: new Date().toISOString().split('T')[0] });
    };

    const handleSubmit = (e) => {
        e.preventDefault();
        if (editingPedido) {
            updateItem('pedidos', editingPedido.id, formData);
        } else {
            addItem('pedidos', formData);
        }
        resetForm();
    };
    ...
    const columns = [
      ...
      { header: "Total", render: (row) => <span className="font-semibold text-green-600">R$ {row.total.toFixed(2)}</span> },
    ...
    const actions = [
      { icon: Eye, label: "Visualizar", onClick: (row) => console.log("Ver pedido", row) },
      { icon: Edit, label: "Editar", onClick: handleEdit },
    ...
    return (
      <div className="p-6 space-y-6 bg-gradient-to-br from-pink-50/30 to-rose-50/30 min-h-screen">
        ...
        <Modal isOpen={showModal} onClose={resetForm} title={editingPedido ? "Editar Pedido" : "Novo Pedido"} size="lg">
            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <Select label="Cliente" value={formData.cliente} onChange={(e) => setFormData({...formData, cliente: e.target.value})} required>
                        <option value="">Selecione um cliente</option>
                        {data.clientes.map(c => <option key={c.id} value={c.nome}>{c.nome}</option>)}
                    </Select>
                    <Input label="Produtos" value={formData.produtos} onChange={(e) => setFormData({...formData, produtos: e.target.value})} required />
                    <Input label="Total (R$)" type="number" step="0.01" value={formData.total} onChange={(e) => setFormData({...formData, total: e.target.value})} required />
                    ...
                    <div className="space-y-2">
                        {pedidosDoDiaSelecionado.map(pedido => (
                            <div key={pedido.id} className="p-2 border rounded-lg flex justify-between">
                                <span>{pedido.cliente} - {pedido.produtos}</span>
  ...
  const Financeiro = () => {
    ...
    const vendasHoje = data.pedidos.filter(p => new Date(p.data).toDateString() === today.toDateString()).reduce((acc, p) => acc + p.total, 0);
    const vendasUltimoMes = data.pedidos.filter(p => new Date(p.data) >= lastMonth).reduce((acc, p) => acc + p.total, 0);
    const vendasUltimos3Meses = data.pedidos.filter(p => new Date(p.data) >= threeMonthsAgo).reduce((acc, p) => acc + p.total, 0);
    ...
    return (
        <div className="p-6 space-y-6 bg-gradient-to-br from-pink-50/30 to-rose-50/30 min-h-screen">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="bg-green-100 p-6 rounded-2xl shadow-lg"><p className="text-green-800">Caixa</p><h2 className="text-2xl font-bold text-green-800">R$ {caixa.toFixed(2)}</h2></div>
            </div>
            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold text-gray-700">Despesas</h2>
                <Button onClick={() => setShowModal(true)}><Plus/> Nova Despesa</Button>
            </div>
            <Table columns={columns} data={data.despesas} />
            <Modal isOpen={showModal} onClose={resetForm} title="Nova Despesa">
  ...
  const renderCurrentPage = () => {
    switch (currentPage) {
      ...
      case 'relatorios': return <PlaceholderPage title="Relatórios" />;
